...
from services.vision import qr_detector as qr_checker
from connectivity.logging_setup import setup_logging
from core import events as E

try:
    from services.audio.beep_service import beep as _beep
except Exception:
...
def is_online(host: str = "8.8.8.8", count: int = 1, timeout_s: float = 1.0) -> bool:
    try:
        # Windows uses -n, -w(ms); Linux uses -c, -W(s)
        import platform
        if platform.system().lower().startswith("win"):
            r = _run(["ping","-n",str(count),"-w",str(int(timeout_s*1000)),host], timeout=timeout_s+0.5)
        else:
...
# ------- Non-blocking connect attempt (used by offline autoconnect) ---------

def attempt_connect(bus: "Any", ssid: str, password: Optional[str], security: Optional[str] = None) -> None:
    """Spawn a background thread that tries to connect using nmcli.
    Emits E.NET_CONNECTING, then E.NET_ONLINE on success or E.NET_CONNECT_FAILED on failure.
    """
    setup_logging()
    bus.emit(E.NET_CONNECTING, {"ssid": ssid})

    def _worker():
        r = _nmcli_connect(ssid, password, hidden=False)
        if not r.ok:
            bus.emit(E.NET_CONNECT_FAILED, {"ssid": ssid, "rc": r.rc, "err": r.err})
            return
        # confirm online quickly
        deadline = time.time() + 8.0
        while time.time() < deadline:
            if is_online():
                bus.emit(E.NET_ONLINE, {"online": True, "ssid": ssid})
                return
            time.sleep(0.6)
        bus.emit(E.NET_CONNECT_FAILED, {"ssid": ssid, "rc": 0, "err": "no_internet_after_connect"})

    th = threading.Thread(target=_worker, name=f"nmcli-connect-{ssid}", daemon=True)
    th.start()
