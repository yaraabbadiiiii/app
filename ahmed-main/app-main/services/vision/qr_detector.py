...
except Exception:  # OpenCV not available
...
def _resolve_wifi_parser():
    ...
    try:
        ...
        pass
    # Project currently has a misspelled file name 'qr_provisionong.py'. Try it too.
    try:
        from connectivity import qr_provisionong as qp2  # type: ignore
        for name in ("parse_wifi_qr", "parse_wifi_payload", "parse"):
            fn = getattr(qp2, name, None)
            if callable(fn):
                return fn
    except Exception:
        pass
